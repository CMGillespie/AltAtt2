...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function handleWebSocketOpen() {
    ...
    const connectRequest = { type: 'connect', presentationCode: state.sessionId, languageCode: state.language, identifier: `secure-viewer-${Math.random().toString(16).substring(2, 8)}` }; 
    if (state.passcode) { 
  ...
  function handleWebSocketMessage(event) { 
    try { 
        ...
        switch (message.type) { 
            ...
            case 'echo': console.log(`Echo received.`); break; 
            default: console.warn(`Unhandled message type: ${message.type}`, message); 
        } 
    } catch (error) { 
