// Wordly Secure Viewer Script (v5 - with Wake Lock, Media Session, Silent Loop, Reconnection, No Timestamps)
document.addEventListener('DOMContentLoaded', () => {

  // --- Service Worker Registration ---
  // Helps keep the audio alive in the background.
  ...
  function handleWebSocketMessage(event) {
    try {
      ...
      switch (message.type) {
        case 'status': handleStatusMessage(message); break;
        case 'phrase': handlePhraseMessage(message); break;
        case 'speech': handleSpeechMessage(message); break;
        case 'users': break; // Do nothing
        case 'end': handleEndMessage(message); break;
        case 'error': handleErrorMessage(message); break;
        case 'echo': break; // Keepalive
        default: console.warn(`Unhandled message type: ${message.type}`);
      }
    } catch (error) {
  ...
  function handleStatusMessage(message) {
    if (message.success) {
      ...
      if (state.websocket) {
        state.websocket.close(1011, "Status error received");
      }
    }
  }
  ...
  function handlePhraseMessage(message) {
    ...
    if (!phraseElement) {
      ...
      // --- MODIFIED: Timestamp span is removed from this innerHTML ---
  ...
  function handleSpeechMessage(message) {
    ...
    if (message.synthesizedSpeech && message.synthesizedSpeech.data && message.synthesizedSpeech.data.length > 0) {
      const audioData = message.synthesizedSpeech.data;
